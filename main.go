...
func (g *Game) LoadLevel(idx int) {
	...
	lvl := g.Levels[idx]
	w := g.World
	w.Physics = make(map[core.Entity]*components.Physics) // Reset physics state
	...
	w.Physics[g.SpectreID] = &components.Physics{MaxSpeed: 6.0, Friction: 0.96, Mass: 1.0}
	w.Renders[g.SpectreID] = &components.Render{Sprite: g.SpriteSpectre, Color: color.RGBA{255, 50, 50, 255}, Glow: true}
	w.AIs[g.SpectreID] = &components.AI{ScriptPath: "spectre.lua"}
	...
	w.AIs[g.RunnerID] = &components.AI{ScriptPath: "runner.lua"}
...
func (g *Game) Update() error {
	...
	systems.SystemInput(g.World, g.RunnerID)
	systems.SystemAI(g.World, g.RunnerID, lvl)
	systems.SystemAI(g.World, g.SpectreID, lvl)
	systems.SystemPhysics(g.World, lvl)
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	systems.DrawLevel(screen, lvl, g.World.Transforms[g.SpectreID].Position)
